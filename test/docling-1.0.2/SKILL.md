---
name: docling
description: Convert local documents (PDF, DOCX, PPTX, XLSX, images) to clean Markdown using the docling CLI. GPU-accelerated OCR and layout analysis. Use when user imports office files into knowledge base or needs to extract content from .pdf/.docx/.pptx/.xlsx files on disk. For web content, use WebFetch (public pages) or kimi-webbridge (pages requiring login).
version: 1.1.0
metadata:
  requires:
    bins: ["docling"]
