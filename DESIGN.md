...
- **下载屏 = 发布清单**：左列是「为什么下载」+ 三条承诺芯片（MIT 开源 / 数据本地 / 永久免费）；右列是一张 RELEASE 面板（版本号由 latest.json 实时写入，两行平台 + 文件名 + 按钮，Windows 走印章红、macOS 走墨色）；下方「更多入口」用编号清单（01–05）代替匿名药丸。整屏仍是纸底，不加深色带。
