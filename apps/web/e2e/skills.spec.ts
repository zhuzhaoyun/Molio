 ...
 * wechat-article-extractor) are app-owned, hidden by the API, and always
 * effective — so these tests assert their ABSENCE and otherwise work with
 * self-created skills. Tests that create skills delete them again to avoid
 * polluting the library. (remotion used to be bundled but was retired — users
 * install the hub's am-will/remotion instead; a remotion row may legitimately
 * exist on machines where it was hub-installed, so no absence assertion for
 * it here. The same applies to the retired core writing trio ids.)
 *
