...
wiki 相关内容的目录结构：
- \`wiki/INDEX.md\` — 递归 Wiki 的根索引，列出顶层主题及其摘要。
...
页面路径规则：
- 嵌套页面和同名文件必须使用带显示别名的路径限定链接，例如 \`[[建筑工程/规范审查/消防规范/concepts/防火分区|防火分区]]\`
- 唯一的 legacy 页面仍可使用不带路径的链接
...
1. **先读 hot.md**（如果存在）：近期上下文缓存。
2. **读 wiki/INDEX.md**：列出候选主题及摘要。
3. 选中条目指向另一个主题 \`INDEX\` 时，逐级读取候选主题 \`INDEX\`。
4. 到达叶主题后，读取相关页面条目或相关索引分片（\`index-shards/\`）。
5. 打开最相关的 Wiki 页面和关联页面。
6. Wiki 信息不足时才读取源文件。

嵌套页面和同名文件必须使用带显示别名的路径限定链接，例如 \`[[建筑工程/规范审查/消防规范/concepts/防火分区|防火分区]]\`。唯一的 legacy 页面仍可使用不带路径的链接。
