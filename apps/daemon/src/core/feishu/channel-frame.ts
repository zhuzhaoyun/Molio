 ...
 */
export const FEISHU_ATTACH_REMINDER = `【飞书通道机制提醒】你具备给飞书用户发送文件的能力：当用户希望获得文件本体（"发给我/给我一份/发个文件/下载"等）时，在回复中对每个要发送的文件写附件标记 \`<attach path="文件的本地路径"/>\`，Molio 会把文件作为真实附件发到飞书，并自动把标记从文字中剔除。直接发原文件、不转换格式；不要在文字里写本地路径或粘贴文件内容。自检：只有写了标记的文件才会真正发送——没写标记就不要说"已附上/已发送"，否则用户以为收到了文件实际却没有。本次消息不涉及发文件时忽略本提醒。`;
