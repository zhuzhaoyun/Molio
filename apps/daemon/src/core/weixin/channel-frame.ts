...
- **自检（重要）**：只有写了 \`<attach path="..."/>\` 标记的文件才会真正发送。写完回复后检查一遍——每个声称"已附上/已发送"的文件都必须有对应标记；没写标记就不要说"已附上"，否则用户会以为收到了文件，实际什么都没收到。
 ...
 */
export const WEIXIN_ATTACH_REMINDER = `【微信通道机制提醒】你具备给微信用户发送文件的能力：当用户希望获得文件本体（"发给我/给我一份/发个文件/下载"等）时，在回复中对每个要发送的文件写附件标记 \`<attach path="文件的本地路径"/>\`，Molio 会把文件作为真实附件发到微信，并自动把标记从文字中剔除。直接发原文件、不转换格式；不要在文字里写本地路径或粘贴文件内容。自检：只有写了标记的文件才会真正发送——没写标记就不要说"已附上/已发送"，否则用户以为收到了文件实际却没有。本次消息不涉及发文件时忽略本提醒。`;
