...
- **加粗书写规范（CJK）**：`**加粗**` 的首尾要紧贴文字，别让括号/引号/逗号等标点落在加粗边缘、外侧又紧贴汉字——`**生态位（萝卜坑）**的分配`（闭合侧失效）、`甲**「重点」**乙`（两侧都失效）这类写法会被 CommonMark 渲染器判定加粗失效、`**` 按字面显示（中文没有空格，极易踩中）。改法：调整语序让加粗首尾都是文字；或在贴标点的那一侧补半角空格，把标点和外侧汉字隔开——闭合侧失效在闭合 `**` 后补（`**生态位（萝卜坑）** 的分配`），开启侧失效在开启 `**` 前补（`甲 **「重点」** 乙`），两侧都失效就两侧都补。
