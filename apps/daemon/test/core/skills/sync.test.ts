...
describe('skills/sync', () => {
  it('syncSkill writes the library SKILL.md into molio--<id>/', () => {
    const entry = createSkill({ name: 'S', description: '', enabled: false, builtIn: false }, 'body', opts);
  ...
  it('removeSkillSyncDir removes only the namespaced dir', () => {
    const entry = createSkill({ name: 'S', description: '', enabled: true, builtIn: false }, 'body', opts);
