...
describe('acp-errors', () => {
  ...
  describe('formatAcpInitFailure', () => {
    ...
    it('keeps raw format when lastStderr is undefined', () => {
      const msg = formatAcpInitFailure(new Error('some failure'), undefined);
      assert.equal(msg, 'ACP init failed: some failure');
    });

    it('does not crash on process-exit errors without stderr', () => {
      // Reporter scenario: hermes-acp crashes so fast no stderr lands at all.
      const msg = formatAcpInitFailure(new Error('hermes-acp process exited (code=1)'), undefined);
