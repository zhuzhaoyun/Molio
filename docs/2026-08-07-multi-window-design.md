...
| 决策点 | 结论 |
|---|---|
| 1. 新窗口入口 | ① 菜单栏「文件 → 新窗口」+ ⌘N/Ctrl+N 快捷键 ② **KB 标签右键「在新窗口打开」** ③ **KB 文件面板统一「＋新建」下拉**（2026-08-10 UX 迭代：新建笔记/新建文件夹/新窗口三合一；「新窗口」点开 vault 列表，**由用户选择要打开的库**，不克隆当前窗口）④ **macOS Dock 右键「新窗口」+「最近使用的知识库」子菜单**（vault-recency LRU，`userData/vault-recency.json` 持久化，focus 3s 节流刷新）⑤ **Windows 任务栏 Jump List「新窗口」**（`--new-window` → 单实例锁 → second-instance；机制限制无动态子菜单） |
