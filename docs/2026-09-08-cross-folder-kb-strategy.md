...
- **日期**：2026-09-08
- **状态**：已落地（分支 `feat/external-source-roots`；阶段 A/B 完成，阶段 C 出处/作用域语义层待做。实现相对设计的偏差见配套文档「落地偏差」清单）
...
- **衍生**：[公众号文章初稿](../../公众号-不搬家也能成库-初稿.md)（2026-09-10，本地未提交）即以本文第三部分为骨架。
...
- **阶段 A**：让 wiki-build 场景先跑（agent 已能跟 junction，Wiki 可建），验证价值。✅ 完成
- **阶段 B**：落「链接素材根」白名单（见配套设计文档），打通 UI 显示 / 读写 / 监听 / 安全四层。✅ 完成（含仓库管理器交互定论，见设计文档 §6.1；注意「可写挂载」已立项为后续迭代，第一版只读）
- **阶段 C**：补「出处与作用域」原则，解决规则层冲突。⏳ 未做（命名空间已就位，对比页 / 检索带出处未实现）
