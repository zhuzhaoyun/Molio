...
- Wiki 主题按需递归。系统默认优先使用一至两级主题；候选叶主题超过容量上限时继续细分，主题路径安全上限为六级。
- 构建支持中断、恢复、跳过、重试和单文件失败隔离。
- 保留 llm-wiki 的持续编译、交叉链接、来源追踪和知识写回能力。
- 叶主题 INDEX 或它的确定性索引分片完整列出全部 Wiki 页面，供浏览、审计和一期查询使用。
...
4. Agent 根据 inventory、目录名称、标题和轻量采样提出递归主题树及构建批次。系统优先生成一至两级主题，并按叶主题容量继续细分。
...
Skill 按叶主题执行构建。叶主题可以位于主题树的任意层级。同一主题中的批次按顺序运行；不同主题在首版也串行执行。每个批次完成后，工具写入文件级和批次级检查点。

用户取消或 Agent 退出后，下次触发 wiki-build 时，Skill 读取获批计划和检查点，展示剩余工作并请求继续。它不会重建已经成功且源文件指纹未变化的批次。

## 5. Wiki 目录约束与参考结构

下面的目录树只说明层级关系，不是固定模板。`topics/` 不作为额外目录，顶层主题直接位于 `wiki/` 下。主题可按容量继续递归。
...
- `wiki/INDEX.md` 只列顶层主题及短摘要。
- 每个主题可以是中间主题或叶主题。中间主题的 `INDEX.md` 只列直接子主题及短摘要。
- 普通叶主题的 `INDEX.md` 按页面类型列出该主题的全部 Wiki 页面，每页包含链接和一句话摘要。
- 叶主题默认最多包含 200 个页面条目，且 `INDEX.md` 估算不超过 12000 tokens；任一指标超限时，构建计划继续细分该主题。
- 主题路径最多包含六级主题。主题达到六级后仍超限，或内容无法继续进行有意义的语义细分时，工具在叶主题的 `index-shards/` 下生成确定性索引分片。工具先按页面类型分组，再按规范标题和相对路径排序，并写入 `<type>-0001.md`、`<type>-0002.md` 等分片。叶主题 `INDEX.md` 列出每个分片的页面类型、标题范围、条目数和短摘要；每个分片遵守相同容量上限。
- 系统不得仅因主题层级、页面数量或 INDEX 大小超过上限而拒绝构建。
- `log.md`、`hot.md`、`meta` 和 `INDEX.md` 是根目录保留名称。

构建计划决定以下内容：

- 递归主题树的名称、路径、父子关系和说明。
- 叶主题容量策略，包括 `maxLeafPages`、`maxLeafIndexTokens` 和 `maxTopicDepth`。一期默认值分别为 200、12000 和 6。
- 哪些候选叶主题需要继续细分，以及达到安全深度或无法语义细分时采用的确定性索引分片规则。系统不为内容集中的知识库强制创建空层级。
...
每个批次先创建或更新 source 页面，再更新该主题的实体、概念和对比页面。叶主题完成后生成主题摘要；中间主题自底向上读取直接子主题摘要进行汇总。全局 `INDEX.md` 只读取顶层主题摘要。实际页面数或 INDEX token 估算超过获批计划的叶容量时，工具按计划中的确定性规则生成索引分片，不修改主题归属。
...
1. 扫描目标文件并计算内容哈希。
2. 根据全局 INDEX、各级主题 INDEX 和叶主题 INDEX 提出主主题。
3. 用户需要时可调整主题归属。
4. Skill 更新 source 页面及相关知识页面。
5. Skill 更新叶主题 INDEX、受影响的索引分片和从叶主题到根的祖先 INDEX。
...
- 一至六级主题路径、父子关系和保留名称校验。
- 叶主题容量判断、递归细分和确定性索引分片。
- 叶主题 INDEX 与索引分片合并后覆盖全部页面，且没有重复项、缺失项或死链。

### 13.2 集成测试

- 从扫描到计划审批，再到三级及更深主题输出的完整流程。
- 构建中途终止后从最后 checkpoint 恢复。
- ingest 单文件后只更新受影响页面、索引分片和祖先 INDEX。
- query 通过任意深度的分层 INDEX 命中 Wiki，缺口场景再回溯源文件。
- 六级主题仍超过叶容量时生成索引分片，构建继续执行且不丢失页面覆盖关系。
- legacy Wiki 无迁移查询。
...
- 构建不会因主题层级、页面数量或 INDEX 大小超过叶容量而拒绝执行；系统继续细分主题或生成索引分片。
