...
完整的叶主题 `INDEX.md` 用于人工浏览、审计和覆盖率检查。query 默认不把整份叶主题 INDEX 注入 LLM 上下文；FTS5 先定位页面，Agent 再按需读取命中页面及其关联页面。页面目录的完整性与查询上下文的大小由不同机制保证。
...
- 构建完成条件由工具检查：获批计划没有 pending/running 批次，叶主题 INDEX 覆盖全部页面，source 页面存在，FTS 索引版本匹配。
