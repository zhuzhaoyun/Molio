...
- Agent 先按知识领域划分主题。缺少共同知识主题的源文件分别进入不同叶主题，单个源文件可以构成叶主题。
...
4. Agent 根据 inventory、目录名称、标题和轻量采样执行语义分组。缺少共同知识主题的文件分别进入不同主题，文件数量不作为合并不同领域的理由。
5. Agent 对每个语义主题执行容量细分。未超过叶主题容量的主题停止细分；超过容量的主题继续拆成满足容量约束的最少数量子主题。
6. Agent 根据主题树生成构建批次，并在对话中展示主题、文件归属、排除项、风险和预计工作量。
7. 用户要求修改或批准计划。
8. Skill 冻结获批计划的版本，并开始执行。
...
- 语义分组先于容量细分。缺少共同知识主题的源文件不得因文件数量少而合并；单文件叶主题属于有效规划结果。
- 容量阈值只控制同一语义主题是否继续细分。未超过容量上限的叶主题不得为增加层级而继续拆分。
- 叶主题默认最多包含 200 个页面条目，且 `INDEX.md` 估算不超过 12000 tokens；任一指标超限时，构建计划继续细分该主题。
- 容量细分必须产生至少两个子主题，并在满足每个子主题容量上限和语义一致性的前提下使用最少的子主题数量。无法产生两个合格子主题时，工具保留当前叶主题并生成索引分片。
...
- 递归主题树的名称、路径、父子关系和说明。
- 每个主题的语义边界和文件归属理由。计划允许单文件叶主题，但不允许只有一个子主题的中间主题。
...
- 两个不同领域的单文件分别形成叶主题；同一领域且未超过容量的文件保持在一个叶主题中。
- 未超过容量的叶主题停止细分；容量细分不产生单子节点中间主题，并使用满足约束的最少子主题数。
...
- 经济与摩托车维修两个单文件生成两个顶层叶主题，不因文件数量少而合并。
