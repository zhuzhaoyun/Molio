...
> 对比日期：2026-07-18（初始基线）｜修订：2026-07-20、2026-07-22、2026-08-03
>
> **修订说明（2026-08-03）**：当前代码新增「拖拽回弹 + 入场过渡 + 位置持久化」后，用户复测确认：
> 回弹/间距/入场/二次拖拽均已解决；卡顿复测已不明显（降级为可选）。
> **拖拽后整簇漂移也已解决 ✅（2026-08-03 同日，质心锁修复）**：终端机判别实验先定位漂移主因是
> **力层磁铁净动量**（每次拖拽注入，入场终态非平衡只是次要一次性残差），随后 `beginDrag` 恢复
> `forceCenter(按下瞬间质心)` 锁住整簇 → 用户复测"没有漂移了"，E2E 质心锁断言守护。
> 详见 `docs/superpowers/specs/2026-07-28-graph-drag-camera-stability.md` §12.2/§12.4。本表
> 拖拽/相机/入场/性能相关状态行已按 2026-08-03 现状更新，与各章旧结论冲突处以本节修订为准。
>
...
| **入场过渡** | ✅ 冷加载聚团+bloom 绽放+去模糊淡入；暖加载（导航回来/刷新，模块缓存+sessionStorage 命中）快速淡入（2026-08-03）。判别实验确认入场终态非平衡只是**次要**一次性残差（主因在力层磁铁净动量），见 `drag-camera-stability.md` §12.2 | 直接显示 | **Molio 更丰富** |
| **增量更新** | ⚠️ 模块级数据缓存 + sessionStorage 位置持久化（SWR，进页面静默刷新），非文件级实时监听 | ✅ 实时文件监听 |
...
| **缩放流畅度** | ✅ velocity 惯性缩放 + 平移惯性（2026-07-22）；<1000 主线程 / ≥1000 Worker 自适应；拖拽/移动时降质（藏标签、collide 3→1） | Web Worker + inertia，60fps |
...
| 交互 | Molio | Obsidian |
|------|-------|----------|
| 画布平移 | Sigma 内置 + 平移惯性（2026-07-22） | 自研 + inertia |
| 缩放 | Sigma 内置 + velocity 惯性缩放（2026-07-22） | 自研（有惯性） |
| Hover | 高亮邻居 | 高亮邻居 + 过渡 |
| 单击 | 选中（紫色 focus 模式） | 选中 + 详情面板 |
| 双击 | **导航到 /knowledge 打开文件** | 打开文件 |
| 拖拽 | 实时 fx/fy 锁定 | fx/fy 锁定 + 磁铁/拴绳局部流体 + **松手自然回弹**（放开被拖节点 + 撤拴绳 + 向心保持，`NEIGHBOR_TETHER=0.3` 蓄张力；相机冻结稳定✅；回弹已恢复✅；卡顿复测不明显✅；**拖拽后整簇漂移已修复✅**——`beginDrag` 装 `forceCenter(按下瞬间质心)` 锁住整簇；**单击选中不抖动✅**——流体安装延后到拖拽过阈值；见 `docs/superpowers/specs/2026-07-28-graph-drag-camera-stability.md` §12.2/§12.4/§12.5） |
